...
trdl = int(LEN['train']/bsz)
vadl = int(LEN['validation']/bsz)
tsdl = int(LEN['test']/bsz)
...
for i in [j for j in [*range(0,PROJ)] if j in [*range(0,PROJ,8)]]:
...
checkpoint_dir = "./training_dual_gan_lodopab_8x_full/" + name
...
EPOCHS = 60
fit(train_dataset, test_dataset, epochs=EPOCHS, initialEpoch=44)